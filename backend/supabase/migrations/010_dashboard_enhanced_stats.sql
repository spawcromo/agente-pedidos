-- Migration: 010_dashboard_enhanced_stats
-- Comprehensive view for the redesigned Admin Dashboard metrics
...
SELECT
  ...
  (SELECT COUNT(*) FROM delivery_routes WHERE delivery_date = CURRENT_DATE AND status = 'active'
   AND EXISTS (SELECT 1 FROM delivery_stops WHERE route_id = delivery_routes.id AND status = 'pending')) AS rutas_hoy_progreso,
  ...
  (SELECT COUNT(DISTINCT driver_id) FROM delivery_routes WHERE delivery_date = CURRENT_DATE AND status = 'active' 
   AND EXISTS (SELECT 1 FROM delivery_stops WHERE route_id = delivery_routes.id AND status = 'pending')) AS repartidores_en_ruta,
  ...
  (SELECT COUNT(*) FROM profiles p WHERE role = 'repartidor' AND NOT EXISTS (
      ...
      AND EXISTS (SELECT 1 FROM delivery_stops WHERE route_id = dr.id AND status = 'pending')
